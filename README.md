...
### Webアプリ作成時の工夫点
- 食材のグラム単価とコンビニ商品の平均価格（マスターデータ）を比較し、「外食額 - 自炊金額 = 節約額」を算出する計算ロジックを実装。レシピ投稿時にリアルタイム計算される仕組みをhotwired/stimulusで構築し、ユーザーが調理前に「そのレシピの経済的価値」を直感的に判断できるUXを実現しました。

- モチベーション維持のため、Chart.jsを用いて節約の推移を可視化。設定した目標金額に達すると、グラフの境界線が「赤から緑」へ動的に変化し、紙吹雪を降らせる演出を実装しました。単なる管理ツールではなく「達成感を得られるプロダクト」であることを意識しました。

- 自炊継続をサポートするために、メールよりも到達・開封率の高いLINE通知機能を実装。LINE Messaging APIを使用し、Render Cron Jobを用いて、ユーザーが指定した日時にリマインドを自動送信する仕組みを構築し、習慣化を技術側面からサポートしています。

- 「迷わせない導線」を最優先し、オートコンプリート機能や非同期での表示順切り替えをTurboで実装。SPAのようなシームレスな操作感を提供し、使用時のストレスを最小限に抑えました。

- テストコードをRSpecで記述。GitHub Actionsを用いたCI/CDパイプラインを構築し、Push毎の自動テストによって、個人開発ながら継続的な品質担保とデプロイの効率化を意識しました。
